        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-2">
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              {isOwner ? "Your artwork" : `Artwork by ${artist.name}`}
            </h2>
            <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
              {isOwner
                ? "Manage everything attached to your profile, including private work in progress."
          ...
          <div className="rounded-full border border-border/60 bg-muted/40 px-4 py-2 text-sm text-muted-foreground">
            {visibleArtworks.length} piece
            {visibleArtworks.length === 1 ? "" : "s"}
          </div>
        </div>

        {visibleArtworks.length === 0 ? (
          <ArtworkEmptyState
            title={isOwner ? "No artwork yet" : "No public artwork yet"}
            description={
              isOwner
                ? "Start building out your profile with your first piece."
                : "This artist has not published any artwork yet."
            }
            actionHref={isOwner ? "/art/add" : undefined}
            actionLabel={isOwner ? "Add artwork" : undefined}
          />
        ) : (
